...
- User-selected row → post fields: `buildingId` (= `id`), optional `buildingName`, `buildingAddress`, `latitude`, `longitude`.
- **`AppContext`** upserts a **building** record when `buildingId` is not already known (demo seeds use `b1`, `b2`, …; OSM picks use `osm-way-…` etc.) so Search can open a profile for new places.
...
- [`ExpoApp/data/buildings.js`](../ExpoApp/data/buildings.js) exports `initialBuildings` (demo profiles) and `PLACEHOLDER_BUILDING_IMAGE` for OSM-created rows.
